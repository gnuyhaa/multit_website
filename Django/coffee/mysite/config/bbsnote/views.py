...
def index(request):
    ...
    # return render(request, 'review/bbsnote/board_list.html', {'board_list': board_list })
    return render(request, 'review/bbsnote/board_list.html', context)

def detail(request, board_id):
    board = Board.objects.get(id = board_id)
    context = {'board':board}
    return render(request, 'review/bbsnote/board_detail.html',context)
...
def board_create(request):
    if request.method == 'POST':
        ...
        form = BoardForm()
    return render(request, 'review/bbsnote/board_form.html', {'form':form})